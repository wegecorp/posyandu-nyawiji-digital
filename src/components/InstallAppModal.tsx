  ...
  const body =
    guide === 'ios' ? (
      <ul className="space-y-3">
        {stepCard('1', <>Buka <strong>{APP_NAME}</strong> memakai <strong>Safari</strong> (browser bawaan iPhone/iPad).</>)}
      ...
      <ul className="space-y-3">
        {stepCard('1', <>Buka {APP_NAME} di <strong>{browserLabel}</strong>.</>)}
        ...
        {stepCard('4', <>Ketuk <strong>Instal / Tambahkan</strong> untuk konfirmasi. Ikon {APP_NAME} akan tersimpan di layar utama.</>)}
      </ul>
    ) : guide === 'desktop' ? (
      <ul className="space-y-3">
      ...
      <div className="bg-white w-full max-w-md rounded-[28px] shadow-2xl overflow-hidden border border-[#e9edef] flex flex-col max-h-[85vh]">
        ...
        <div className="bg-[#075e54] text-white px-5 pt-5 pb-4 flex items-start justify-between gap-3">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-extrabold text-sm leading-snug">Install Aplikasi {APP_NAME}</h2>
