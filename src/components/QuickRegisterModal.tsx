...
export const QuickRegisterModal: React.FC<QuickRegisterModalProps> = ({
  ...
  const categoryPreview = birthDate ? getPatientCategory(birthDate, isPregnant) : null;
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-lg rounded-t-[28px] sm:rounded-[28px] shadow-2xl overflow-hidden max-h-[92vh] flex flex-col border border-[#dee3e9]">
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 flex-1">
          ...
          <div className="flex items-center justify-between p-3.5 bg-rose-50/70 border border-rose-200 rounded-2xl">
            <div className="flex items-center gap-2.5">
              <Heart className="w-4 h-4 text-rose-500" />
              <div>
                <span className="text-xs font-bold text-[#0f172a]">Pasien Ibu Hamil (Bumil)</span>
                <p className="text-[11px] text-[#64748b]">Form akan mengaktifkan kolom kehamilan & LiLA</p>
              </div>
            </div>
            <input
              type="checkbox"
